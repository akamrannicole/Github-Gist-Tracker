...
export default function EditGistPage() {
  ...
  const onSubmit = async (data: GistFormValues) => {
    try {
      ...
      // If filename has changed, mark the old one for deletion and add the new one
      if (data.filename !== originalFilename) {
        files[originalFilename] = null // Mark for deletion
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <div className="flex justify-end gap-2">
                  <Button type="button" variant="outline" onClick={() => router.push(`/gists/${id}`)}>
                    Cancel
                  </Button>
